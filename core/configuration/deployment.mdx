...
CORS_ORIGIN=https://your-frontend.com  # Optional: restrict API access to specific origins
STREMIO_ADDON=true  # Optional: enable Stremio addon endpoint at /stremio/manifest.json
...
Official multi-architecture images are published to the GitHub Container Registry: https://github.com/cinepro-org/core/pkgs/container/core. Use the official image in Docker Compose by setting the service image to `ghcr.io/cinepro-org/core:latest` (or run `docker compose pull` before `up` to fetch the latest remote image). Both linux/amd64 and linux/arm64 platforms are supported.
...
services:
  cinepro-core:
    image: ghcr.io/cinepro-org/core:latest
    # optional: request a specific platform if your host requires it
    ...
    environment:
      ...
      - CACHE_TYPE=redis
      # enable the Stremio addon from the container
      - STREMIO_ADDON=true
    depends_on:
      - redis
```
...
Official multi-architecture images are published to the GitHub Container Registry: https://github.com/cinepro-org/core/pkgs/container/core. Pull the latest official image with:

```bash
# default latest
docker pull ghcr.io/cinepro-org/core:latest

# explicit platforms (linux/amd64 and linux/arm64 supported)
docker pull --platform linux/amd64 ghcr.io/cinepro-org/core:latest
docker pull --platform linux/arm64 ghcr.io/cinepro-org/core:latest
```

When running the official image, pass required environment variables (TMDB_API_KEY, CACHE_TYPE, etc.) and enable the Stremio addon if desired with `-e STREMIO_ADDON=true`.
