...
CinePro Core can also pull sources from third-party Stremio addons and surface them through its own provider pipeline. The list of addons is configured in code (not via environment variables), inside the `stremio` block in `src/server.ts`.

By default the `stremioAddons` array is **empty** — CinePro ships without any pre-wired Stremio addons. To use one, add an entry with a unique `id`, the addon's `manifest.json` URL, and an `enabled` flag.
...
| --- | --- | --- |
| `id` | `string` | Unique identifier for the addon. Used internally to namespace results. |
| `url` | `string` | Full URL to the addon's `manifest.json`. |
| `enabled` | `boolean` | Toggle the addon without removing the entry. |

<Note>
  Configured Stremio addons are only consumed when `STREMIO_ADDON=true`. The `enableNativeAddon` flag and the `stremioAddons` array are independent — you can expose a Stremio addon, consume external Stremio addons, both, or neither.
</Note>
