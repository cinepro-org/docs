...
description: 'Read the full CinePro Core changelog with release notes, new features, bug fixes, and breaking changes for every version of the streaming backend.'
mode: 'wide'
---
...
<Update label="May 19, 2026" tags={["Update", "Fix"]}>
  ...
  - **Broader stream pattern matching** — Added `trendimovies.com/tgstream/stream` and a generic `/content/.../page-*.html` matcher to the recognized streaming-pattern list, so Peachify links served from those CDNs are properly proxied during playback.
  ...
  - **Codeberg mirror** — A new GitHub Actions workflow now mirrors the CinePro Core repository to Codeberg every six hours and on every push, giving contributors a backup remote outside GitHub.
  - **Single-provider testing recipe** — The [contributing guide](https://github.com/cinepro-org/core/blob/main/.github/CONTRIBUTING.md) now documents how to add a per-provider `test.ts` script and run it with `npx tsx`, so you can iterate on one provider without booting the full server.
</Update>
