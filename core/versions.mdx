...
<Update label="May 10, 2026" tags={["Feature", "Fix"]}>
  ...
  - **CinePro UI public beta** — On startup, CinePro Core now prints a banner pointing you to the new [CinePro UI](https://ui.cinepro.cc/) public beta. The banner pre-fills the UI with your server's `PUBLIC_URL` (or the configured `HOST` and `PORT`), so you can try the web frontend against your own instance with a single click. Contributors are also welcome at the [CinePro UI repository](https://github.com/cinepro-org/ui).

  ## Bug fixes

  - **Readable release notes** — Automated GitHub Releases now render with proper line breaks and section formatting again. Previous releases collapsed everything onto a single line, making the categorized notes (Features, Bug Fixes, etc.) hard to read. New releases ship with the intended layout.
</Update>
