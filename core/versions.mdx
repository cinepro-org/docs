...
<Update label="May 10, 2026" tags={["Feature"]}>
  ## New features

  - **CinePro UI public beta** — CinePro Core now prints a startup banner pointing to the new [CinePro UI](https://ui.cinepro.cc), a web frontend that talks to your running instance over its OMSS API. The banner generates a ready-to-use link of the form `https://ui.cinepro.cc/?omssurl=<your-instance-url>`, derived from your [`PUBLIC_URL`](/core/configuration/environment#param-public-url) (or `HOST` and `PORT` if `PUBLIC_URL` is unset). The UI is in public testing and is looking for contributors — see the [cinepro-org/ui repository](https://github.com/cinepro-org/ui) to get involved.
</Update>
