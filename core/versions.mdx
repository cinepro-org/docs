---
title: 'Changelog'
description: 'CinePro Core changelog with release notes, new features, bug fixes, and breaking changes for every version of the streaming backend.'
mode: 'wide'
---
...
<Update label="June 22, 2026" tags={["UI", "Update"]}>
  ## CinePro/ui updates

  - **Faster first-load splash** — The startup overlay on [CinePro/ui](https://ui.cinepro.cc) no longer blocks real content behind a blur for several seconds on first visit. The brand moment is preserved, but the artificial wait has been cut so the app feels noticeably lighter on entry. The splash continues to show only once per browser session.
  - **Sharper glass effects across screen sizes** — The glass refraction filter used on header and drawer surfaces now scales its displacement to the element's own size instead of using fixed pixel values tuned for small buttons. Wide pills and large panels no longer look melty or distorted, while smaller controls keep their original look.
</Update>

<Update label="June 18, 2026" tags={["UI", "Update"]}>
  ## CinePro/ui updates

  - **Back button in stacked media drawers** — When you open a media drawer from inside another drawer (for example, drilling into a recommendation while already viewing a title in [CinePro/ui](https://ui.cinepro.cc)), a `Back` arrow now appears in the drawer header so you can return to the previous layer without dismissing the whole stack. The button only shows when there is a previous drawer to go back to, so single-level drawers are unaffected.
  - **Polished media drawer visuals** — The media drawer now uses a multi-layer overlay with refined gradients, a clearer title and meta block, animated action buttons, and a drop-shadowed overview. Genre badges are capped at three for readability, and button and icon sizes have been tuned for better touch targets and accessibility.
</Update>
...
<Update label="May 21, 2026" tags={["Feature", "Update"]}>
  ...
  - **Tulnex provider** — A new `tulnex` provider has been added to the [provider lineup](/core/configuration/providers) and is enabled by default. It fans out across 14 upstream Tulnex servers (including `onion`, `vidzee`, `icefy`, `vaplayer`, multiple `vidfast-*` variants, `allmovies`, `vidlink`, and `moviebox`) in parallel, decrypts each payload, and aggregates every playable stream into a single result for both movies and TV.
  - **One-click Vercel deployments** — CinePro Core now ships with a `vercel.json` so you can deploy the backend as a serverless function on Vercel in a few clicks. See the new **Vercel** tab in the [quickstart](/quickstart) for the full flow, including how to wire up `TMDB_API_KEY` and `PUBLIC_URL`.
  - **`INTERNAL_DEBUG` flag** — A new [`INTERNAL_DEBUG`](/core/configuration/environment#param-internal-debug) environment variable returns every source the providers discover (including non-playable ones) and attaches per-provider diagnostics. Intended for provider development and debugging — leave it `false` in normal use.

  ## Updates

  - **Every returned source is now playable** — In normal mode (`INTERNAL_DEBUG=false`), the API filters out unplayable candidates upstream so clients only see streams that should actually play.
  - **Cleaner VidApi responses** — The `vidapi` provider now uses a stable desktop user agent and includes an explicit `Accept` header, improving compatibility with upstream stream URL filtering.
  - **Normalized VidNest quality labels** — VidNest stream qualities are now passed through the shared quality inference so labels (e.g. `1080p`, `720p`) match the rest of the providers.
</Update>

<Update label="May 20, 2026" tags={["Feature"]}>
  ## New features

  - **VidApi provider** — A new `vidapi` provider has been added to the [provider lineup](/core/configuration/providers) and is enabled by default. It contributes additional movie and TV sources by querying the upstream VidApi endpoint with the title's TMDB ID (plus season and episode for TV) and returns playable HLS or MP4 streams alongside any default subtitle tracks (`vtt`, `srt`, `ass`, `ssa`, or `ttml`). Stream URLs are routed through CinePro's built-in proxy so the required headers are forwarded automatically — no configuration changes required.
</Update>
