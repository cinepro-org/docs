...
<Update label="May 20, 2026" tags={["Feature"]}>
  ...
  - **VidApi provider** — A new `vidapi` provider has been added to the [provider lineup](/core/configuration/providers) and is enabled by default. It contributes additional movie and TV sources via the VaPlayer API, auto-detects HLS vs. MP4/MKV stream types, infers quality from the upstream file name, and surfaces any default subtitle tracks (`vtt`, `srt`, `ass`, `ssa`, `ttml`) with no configuration changes required.
</Update>
