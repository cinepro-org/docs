...
<Update label="April 27, 2026" tags={["Feature", "Update"]}>
  ...
  - **Popr provider** — A new built-in [streaming provider](/core/configuration/providers), Popr, has been added and is enabled by default. It expands the pool of available sources for both movies and TV shows, with subtitles supported out of the box.
  - **Optional MCP support** — CinePro Core now ships with optional MCP (Model Context Protocol) support so you can plug it directly into your AI agents. Opt in by setting `MCP_ENABLED=true` in your [environment](/core/configuration/environment).
